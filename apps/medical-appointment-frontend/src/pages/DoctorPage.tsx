...
const appointments = [
    { id: 1, patient: 'Петров Алексей Иванович', age: 31, date: '24 фев 2026', time: '09:00', status: 'upcoming', complaint: 'Головная боль, температура' },
    { id: 2, patient: 'Сидорова Мария Андреевна', age: 45, date: '24 фев 2026', time: '10:00', status: 'upcoming', complaint: 'Плановый осмотр' },
    { id: 3, patient: 'Кузнецов Игорь Олегович', age: 28, date: '24 фев 2026', time: '11:30', status: 'completed', complaint: 'Боль в спине' },
    { id: 4, patient: 'Фёдорова Ольга Николаевна', age: 55, date: '25 фев 2026', time: '09:30', status: 'upcoming', complaint: 'Давление, слабость' },
    { id: 5, patient: 'Морозов Виктор Сергеевич', age: 38, date: '25 фев 2026', time: '11:00', status: 'cancelled', complaint: 'Кашель, насморк' },
]
...
export default function DoctorPage() {
    ...
    const todayAppointments = appointments.filter(a => a.date === '24 фев 2026')
    ...
    return (
        <div className="min-h-screen bg-[#dbeafe]">
                ...
                <div className="max-w-4xl mx-auto px-4 h-14 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-3">
                        <div className="hidden sm:flex flex-col items-end">
                            <span className="text-xs font-medium text-slate-700">{doctor.fio}</span>
            ...
            <main className="max-w-4xl mx-auto px-4 py-6">
                ...
                <div className="mb-6">
                    <h1 className="text-xl font-bold text-slate-900">Добрый день, {doctor.fio.split(' ')[1]} 👨‍⚕️</h1>
                ...
                <div className="grid grid-cols-3 gap-3 mb-6">
                    {[
                        ...
                        { label: 'Предстоящих', value: upcomingAppointments.length, color: 'text-indigo-600' },
                        { label: 'Рейтинг', value: doctor.rating, color: 'text-amber-500' },
                    ].map(stat => (
                            ...
                            >
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs text-slate-400">{a.age} лет · {a.complaint}</p>
                                    </div>
                                </div>
                            ...
                            <div key={a.id} className="bg-white rounded-2xl px-5 py-4 flex items-center justify-between shadow-sm border border-blue-50">
                                <div className="flex items-center gap-4">
                                    <div className="flex flex-col items-center justify-center h-10 w-14 rounded-xl bg-blue-50 text-blue-700">
                                        ...
                                        <span className="text-[10px] text-blue-400 leading-none mt-0.5">{a.date.split(' ').slice(0,2).join(' ')}</span>
                                    </div>
                                    <div>
                                        <p className="text-sm font-semibold text-slate-800">{a.patient}</p>
                                        <p className="text-xs text-slate-400">{a.complaint}</p>
                                    </div>
                                </div>
                                <span className={`text-xs font-medium px-2.5 py-1 rounded-full ${statusMap[a.status as keyof typeof statusMap].className}`}>
                  {statusMap[a.status as keyof typeof statusMap].label}
                ...
                {activeTab === 'profile' && (
                    <div className="bg-white rounded-2xl p-6 shadow-sm border border-blue-50 max-w-md space-y-5">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <p className="font-semibold text-slate-800">{doctor.fio}</p>
                        ...
                        <div className="space-y-3">
                            {[
                                ...
                                { label: 'Опыт работы', value: `${doctor.experience} лет` },
                                { label: 'Рейтинг', value: `⭐ ${doctor.rating}` },
                            ].map(f => (
            ...
            {selectedAppointment && (
                <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center px-4 z-50">
                    <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm p-6">
                        ...
                        <div className="space-y-3">
                            {[
                                { label: 'Пациент', value: selectedAppointment.patient },
                                { label: 'Возраст', value: `${selectedAppointment.age} лет` },
                                { label: 'Дата и время', value: `${selectedAppointment.date} в ${selectedAppointment.time}` },
                                { label: 'Жалоба', value: selectedAppointment.complaint },
                            ].map(f => (
                                <div key={f.label} className="flex flex-col gap-0.5">
                                    <span className="text-xs text-slate-400 uppercase tracking-wide">{f.label}</span>
                                    <span className="text-sm font-medium text-slate-700">{f.value}</span>
